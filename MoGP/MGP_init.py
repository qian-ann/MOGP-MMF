...
for idx in range(0,100):
    arg_dic[f'ARG{idx}']=round(0.01*idx,2)
    ...
    for node in individual:
        ...
        if 'ARG' in node.name:
            if node.name not in dic1:
                ...
                used_inputs+=1+arg_dic[node.name]
                idx = int(node.name[3:])
                if dic[idx][:2] not in dic0:
                    dic0.append(dic[idx][:2])
                    used_inputs += 10
    used_inputs += len(individual)/10   #引入个体节点个数增加第二个指标的多样性
...
def init_GP(para,GPpara,data,mode='train'):
    ...
    def evalTrain(individual):
        try:
            # 将个体编译成可运行的函数
            if not hasattr(individual,'func'):
            ...
            failureVector=0
            ObjValueN=0
        usefe_num = count_used_inputs(individual, pset)
        return (round(accuracy,5), usefe_num), failureVector, ObjValueN
        ...
        def evalTest(population, para):
            ...
            for namei in datasetname:
                ...
                with torch.no_grad():
                    ...
                    if not hasattr(para,'idx'):
                        y_pred = ySet[-1]
                    else:
                        y_pred = ySet[para.idx]
                    ...
                    for yidx in range(min(round(len(population)/2),len(ySet)-1)):  # 集成10个
                        y_pred += ySet[-2 - yidx]
            ...
            if not hasattr(para,'yset'):
                with open(rf'./Y_pred_and_Y_targ/y_pred_Set_{para.acc}.pkl', 'wb') as f:
