...
def varAnd(population, toolbox, cxProb, mutProb):  #执行变异和交叉
    ...
    while i < len(offspring):
        if random.random() < new_cxProb:  #交叉变异，如果相同就对两者进行变异操作
            ii=i+1
            while ii<len(offspring) and ((offspring[i] == offspring[ii]) or pop_compare(offspring[i], offspring[ii])):
                ii += 1
            if ii==len(offspring):
...
def eaSimple(population, toolbox, GPpara, stats=None,
    ...
    hof_store = tools.HallOfFame(5 * len(population))  # 创建了一个新的名人堂 hof_store，其最大容量是当前代种群的个体数的 5 倍
    ...
    for gen in range(1, GPpara.generation + 1):
        ...
        elitismNum = int(numpy.ceil(GPpara.elitismProb * len(population)))
        ...
        offspring = toolbox.select(population, len(population)-len(offspringE))
        ...
        offspring = varAnd(offspring, toolbox, GPpara.cxProb, GPpara.mutProb)
        ...
        # # Select the next generation individuals
        # population[:] = toolbox.select(offspring, len(population)-len(offspringE))
        ...
        # [population_sort[idx].fitness.values for idx in range(0, len(population_sort))]
        # [population[idx].fitness.values for idx in range(0, len(population_sort))]
        # [pop2[idx].fitness.values for idx in range(0, len(population_sort))]
        # Append the current generation statistics to the logbook
        record = stats.compile(population) if stats else {}
        logbook.record(gen=gen, nevals=len(population), **record)
        print(logbook.stream)
    pop2 = toolbox.selectElitism(hof_store, k=len(population))
    return population, logbook, halloffame, bestacc
